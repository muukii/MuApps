...
/// Emoji catalog used by the vault icon picker.
///
/// The values are generated from Unicode emoji-test.txt fully-qualified rows so
/// selecting a new emoji does not require changing the vault catalog schema.
enum VaultEmojiCatalog {

  /// One primary picker item and every exact Unicode value selectable from it.
  ///
  /// Skin-tone modifiers are removed only for grouping. `baseValue` and
  /// `values` preserve the fully-qualified strings from `emoji-test.txt`.
  struct Family: Hashable, Identifiable, Sendable {

    /// Stable grouping key with skin-tone modifiers and variation selectors removed.
    let id: String

    /// Preferred neutral form, or the first fully-qualified value when none exists.
    ...
    /// Returns whether an optional selected emoji belongs to this family.
    func contains(_ selectedEmoji: String?) -> Bool {
      guard let selectedEmoji else { return false }
      return values.contains(selectedEmoji)
    }

    /// Keeps a selected skin-tone value visible in the collapsed primary grid.
    func displayValue(selectedEmoji: String?) -> String {
      guard let selectedEmoji, contains(selectedEmoji) else {
        return baseValue
      }
  ...
  /// Primary picker items with skin-tone variants collapsed into each family.
  ///
  /// This collection is prepared once, on first entry into the dedicated Emoji
  /// browser, instead of mapping all catalog values whenever the vault form lays out.
  static let families: [Family] = {
    var orderedKeys: [String] = []
    var valuesByKey: [String: [String]] = [:]

    for value in all {
      let key = familyKey(for: value)
    ...
    return orderedKeys.compactMap { key in
      guard let values = valuesByKey[key], let firstValue = values.first else {
        return nil
      }
  ...
  private static func familyKey(for value: String) -> String {
    let normalizedKey = String(
      value.unicodeScalars.filter { scalar in
        isSkinToneModifier(scalar) == false && scalar.value != 0xFE0F
      }
    )
    return canonicalFamilyKeyAliases[normalizedKey] ?? normalizedKey
  }

  /// Unicode uses expanded person sequences only for the mixed-tone forms of
  /// these legacy emoji. The aliases keep those forms under the neutral item.
  private static let canonicalFamilyKeyAliases: [String: String] = [
  ...
  private static func containsSkinToneModifier(_ value: String) -> Bool {
    value.unicodeScalars.contains(where: isSkinToneModifier)
  }

  private static func isSkinToneModifier(_ scalar: Unicode.Scalar) -> Bool {
